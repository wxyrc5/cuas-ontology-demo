...
def show() -> None:
    ...
    st.markdown(
        """
        在本体之上跑 **SPARQL 查询**，推理出 **杀伤链、装备对抗、人员配置** 等关键作战结论。
    ...
    if "custom_query" not in st.session_state:
        st.session_state["custom_query"] = (
            ...
            "  ?e a cuas:Edge ; cuas:linkType cuas:mission_protects_asset ;\n"
    ...
    cust = st.text_area(
        ...
        height=180,
        help="前缀建议：PREFIX cuas: <https://cuas.ontology.org/>",
    )
    ...
    with st.expander("📚 本体前缀与变量参考", expanded=False):
        st.markdown(
            ...
            | 前缀 | 命名空间 | 说明 |
            |------|---------|------|
            | `cuas:` | `https://cuas.ontology.org/` | 本体主命名空间 |
            ...
            **常用类 / 属性**：
            ...
            - 类：`cuas:ObjectType` `cuas:LinkType` `cuas:Edge`
            - 属性：`cuas:linkType` `cuas:from_instance` `cuas:to_instance`
            - 关系：`cuas:mission_protects_asset` 等 10 个 Link Type 名称
            - 实例：`MSN-001` · `EQP-RDR-01` · `THR-003` · `AST-001` · `OPR-001` 等
            """
        )
